...
class TOTALVI_shared(nn.Module):
    ...
    def inference(
        ...
        # Sampling - Encoder gets concatenated genes + proteins
        additional_columns = torch.tensor([[0.0, 1.0]] * x_.shape[0], dtype=torch.float32,device='cuda')
        x_extended = torch.cat([x_, additional_columns], dim=1)
        additional_columns = torch.tensor([[1.0, 0.0]] * y_.shape[0], dtype=torch.float32,device='cuda')
