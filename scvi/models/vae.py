...
class VAE(nn.Module):
    ...
    def __init__(
        ...
        if self.dispersion == "gene":
            ...
            self.px_r = torch.nn.Parameter(torch.randn(n_output, n_batch))
        elif self.dispersion == "gene-label":
            self.px_r = torch.nn.Parameter(torch.randn(n_output, n_labels))
    ...
    def get_reconstruction_loss(
        ...
        if self.reconstruction_loss == "zinb":
            ...
            reconst_loss = -Poisson(px_rate).log_prob(x_).sum(dim=-1)
        return reconst_loss
    ...
    def forward(
        ...
        return reconst_loss + kl_divergence_l + kl_divergence, z

class VAE_shared(nn.Module):
    """Variational auto-encoder model.
