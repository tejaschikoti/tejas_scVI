...
class DecoderTOTALVI_shared(nn.Module):
    ...
    def forward(self,gene_size: int,z: torch.Tensor, library_gene: torch.Tensor, *cat_list: int):
        r"""The forward computation for a single sample.
        ...
        px_["dropout"] = self.px_dropout_decoder_gene(z_gene, *cat_list)
        py_["mixing"] = self.py_background_decoder(p_mixing_cat_z, *cat_list)
...
class EncoderProteinVI(nn.Module):
    r"""Encodes data of ``n_input`` dimensions into a latent space of ``n_output``
    dimensions using a fully-connected neural network of ``n_hidden`` layers.
...
class DecoderproteinVI(nn.Module):
    r"""Decodes data from latent space of ``n_input`` dimensions ``n_output``
    dimensions using a linear decoder
