...
from scvi.models.modules import DecoderTOTALVI, EncoderTOTALVI,DecoderTOTALVI_shared
...
class TOTALVI_shared(nn.Module):
    ...
    def inference(
        ...
        orig_tensor = torch.cat((x_, y_), dim=0)
        ...
        latent["l"] = latent["z"][torch.argsort(permuted_indices)]
        untran_latent["z"] = untran_latent["z"][torch.argsort(permuted_indices)]
        untran_latent["l"] = untran_latent["z"][torch.argsort(permuted_indices)]
        

        z = latent["z"]
        library_gene = latent["l"]
        print(f"Ok to check {library_gene.shape}")
        ...
        print(f"Modified here to check {library_gene.shape}")
